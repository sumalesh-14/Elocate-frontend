        ...
        <div className="animate-fade-in-up space-y-6">
          ...
          <div className="flex gap-4 border-b border-gray-200">
            {['pending', 'rejected'].map((status) => (
              <button
                ...
                className={`pb-3 px-2 text-sm font-medium capitalize transition-all relative ${approvalTab === status
                    ? 'text-eco-900'
                    : 'text-gray-400 hover:text-gray-600'
                  }`}
              >
                {status} Applications
                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs ${approvalTab === status ? 'bg-eco-100 text-eco-800' : 'bg-gray-100'
                  }`}>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            {filteredApps.map((app) => (
              <div key={app.id} className={`bg-white p-6 rounded-[1.5rem] border shadow-sm hover:shadow-md transition-all group relative cursor-pointer ${selectedAppIds.includes(app.id) ? 'border-eco-500 ring-1 ring-eco-500' : 'border-gray-100'}`}
                onClick={() => {
                ...
                <div className="absolute top-4 right-4">
                  <input
                    ...
                    className="w-5 h-5 rounded border-gray-300 text-eco-600 focus:ring-eco-500 pointer-events-none"
                  />
                </div>
                    ...
                    {approvalTab === 'pending' && (
                      <>
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 bg-eco-900 text-white rounded-lg hover:bg-eco-800 font-medium text-sm transition-colors shadow-md hover:shadow-lg disabled:opacity-50"
                          disabled={loading}
                        >
