...
import { Search, Filter, MoreHorizontal, Mail, Ban, CheckCircle, MapPin, User, Send, X, Shield, Download, Eye, Edit2, Save, ChevronRight } from 'lucide-react';
...
export const CitizenManagement: React.FC = () => {
  ...
  // Calculate total elements and current page slice
  const total = filteredCitizens.length;
  const paginatedCitizens = filteredCitizens.slice(page * pageSize, (page + 1) * pageSize);
    ...
    <div className="space-y-6 relative">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className={`p-2 rounded-lg transition-colors tooltip ${citizen.status === 'Active'
        ...
        <div className="bg-white px-8 py-6 border-t border-gray-100 flex flex-row items-center justify-between rounded-b-[2rem] shadow-sm gap-4">
          <div className="flex flex-row items-center gap-4 flex-shrink-0">
            <span className="text-sm font-medium text-gray-500 whitespace-nowrap inline-block">
              Showing <span className="text-eco-900 font-bold inline-block">{paginatedCitizens.length}</span> of <span className="text-eco-900 font-bold inline-block">{total}</span> results
            </span>
      ...
      {showDetailsModal && selectedCitizen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 md:left-80 bg-black/30 transition-opacity" onClick={() => setShowDetailsModal(false)}></div>
      ...
      {showEmailModal && selectedCitizen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 md:left-80 bg-black/30 transition-opacity" onClick={() => setShowEmailModal(false)}></div>
          <div className="relative w-full max-w-lg md:ml-80 bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-black/5 overflow-hidden animate-fade-in-up">
